...
#include "Common/Player.h"  // WarPowers @debug WP_AI_TRACE
#include "Common/PlayerTemplate.h"
#include "Common/Team.h"
#include "Common/PlayerList.h"
#include "Common/ThingTemplate.h"  // WarPowers @debug WP_AI_TRACE
#include "GameLogic/Object.h"  // WarPowers @debug WP_AI_TRACE
#include "GameClient/ControlBar.h"  // WarPowers @debug WP_AI_TRACE
#include "GameClient/View.h"  // TheTacticalView (isTimeFrozen)
#include "GameLogic/Module/ProductionUpdate.h"  // WarPowers @debug WP_AI_TRACE
...
#include "WPTrace.h"  // WarPowers: wpEnvEnabled() gate shared by the fork's traces
#include "GameClient/WPShell.h"  // WarPowers: HUD/telemetry refresh entry point
// WarPowers @refactor 07/09/2026 The WP_AUTOTEST / WP_RETRY / WP_CLICKTEST
// state machines live in WarPowers/WPHarness.cpp and are compiled only with
// -DWP_HARNESS=ON, so the shipped engine tick carries no test harness.
#if WP_HARNESS
	...
	{
		{
			...
			// WarPowers @feature music rotation: cycles the GameData MusicRotation
			// list (audio event names) with a short silence between tracks. Runs in
			// menu and match alike and respects the mixer's music volume. Datasets
			// that leave the list empty keep the engine's stock music behaviour.
			// WarPowers @refactor 07/09/2026 track names moved from code to GameData.ini.
			{
				static AudioHandle s_wpMusicHandle = 0;
				static Int s_wpTrackIdx = -1;
				static Int s_wpMusicGap = 150;   // ~5s before the first track
				const std::vector<AsciiString> *rotation = TheGlobalData ? &TheGlobalData->m_musicRotation : nullptr;
				if (TheAudio && rotation && !rotation->empty())
				{
					...
					{
						s_wpTrackIdx = (s_wpTrackIdx + 1) % (Int)rotation->size();
						AudioEventRTS wpTrack((*rotation)[s_wpTrackIdx]);
			...
			{
				// WarPowers @refactor 07/09/2026 gate shares the WPTrace.h env
				// semantics (set, non-empty and not "0") instead of a raw pointer check.
				static const Bool wp_aiTrace = wpEnvEnabled("WP_AI_TRACE");
				if (wp_aiTrace && TheGameLogic && TheGameLogic->isInGame() &&
			...
			// WarPowers @feature 05/09/2026 Refresh the War Powers HUD and the
			// browser mission telemetry without changing simulation state.
			WPUpdatePlayerExperience();
#if WP_HARNESS
			// WarPowers @refactor 07/09/2026 WP_AUTOTEST / WP_RETRY state machines
			// (WarPowers/WPHarness.cpp) run here, after the client update and before
			// the message stream propagates, exactly where they ran inline before.
			WPHarness_Update();
#endif
